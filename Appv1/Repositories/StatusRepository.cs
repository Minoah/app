...
namespace Appv1.Repositories
{
    ...
    public class StatusRepository : IStatusRepository
    {
        ...
        {
            IQueryable<StatusDAO> Statuss = DataContext.Statuses;
        ...
        public async Task<List<Status>> List(StatusFilter filter)
        {
            ...
            IQueryable<StatusDAO> StatusDAOs = DataContext.Statuses;
        ...
        {
            Status Status = await DataContext.Statuses.Where(x => x.Id == Id).Select(x => new Status()
            {
                Id = x.Id,
                Code = x.Code,
                Name = x.Name,
